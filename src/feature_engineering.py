...
def calculate_volatility(main_df, history_df):
    if history_df is None:
        # print("No se pueden calcular las métricas de volatilidad sin datos históricos.")
        main_df['volatilidad'] = 0
        main_df['rendimiento_ajustado'] = main_df['valor']
        return main_df

    # print("Calculando volatilidad y rendimiento ajustado...")
    # Calcular la desviación estándar de los puntos en los últimos 10 partidos como 'volatilidad'
    volatility = history_df.groupby(['Nombre', 'Apellido'])['total_points'].rolling(10, min_periods=1).std().reset_index()
    volatility = volatility.groupby(['Nombre', 'Apellido'])['total_points'].last().reset_index()
    volatility.rename(columns={'total_points': 'volatilidad'}, inplace=True)
    volatility['volatilidad'] = volatility['volatilidad'].fillna(0)

    # Unir la volatilidad al dataframe principal
    merged_df = main_df.merge(volatility, on=['Nombre', 'Apellido'], how='left')
    merged_df['volatilidad'] = merged_df['volatilidad'].fillna(0)

    # Corregir el problema de volatilidad cero
    ...
    # --- 3. Feature Adicional: Rendimiento Ajustado por Riesgo ---
    merged_df['rendimiento_ajustado'] = merged_df['valor'] / merged_df['volatilidad']
    
    # Limpiar valores infinitos o NaN que puedan surgir
    merged_df.replace([np.inf, -np.inf], np.nan, inplace=True)
    merged_df['rendimiento_ajustado'].fillna(0, inplace=True)
